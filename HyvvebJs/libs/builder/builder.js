...
(function() {
  ...
  this.tmpl = function tmpl(str, data) {
    // Figure out if we're getting a template, or if we need to/弄清楚我们是否得到了一个模板，或者我们是否需要
    // load the template - and be sure to cache the result. /加载模板-并确保缓存结果。
    var fn = /^[-a-zA-Z0-9]+$/.test(str)
      ? (cache[str] =
          cache[str] || tmpl(document.getElementById(str).innerHTML))
      : // Generate a reusable function that will serve as a template /生成作为模板的可重用函数
        // generator (and which will be cached). /生成器(将被缓存)
        new Function(
          "obj",
          "var p=[],print=function(){p.push.apply(p,arguments);};" +
            // Introduce the data as local variables using with(){} /使用with(){}将数据作为局部变量引入
            "with(obj){p.push('" +
            // Convert the template into pure JavaScript  /  将模板转换为纯JavaScript
            str
              .replace(/[\r\t\n]/g, " ")
              .split("{%")
              .join("\t")
    ...
    // Provide some basic currying to the user / 为用户提供一些基本的加密
...
//? --------------------------------------------------------------------------------------------------------------------------
if (Hyweb === undefined) var Hyweb = {};

Hyweb.defaultComponent = "_base"; //! 默认组件
Hyweb.preservePropertySections = true;
Hyweb.dragIcon = "icon"; //图标=使用组件图标时拖动| html =使用组件html创建可拖动的元素

Hyweb.baseUrl = document.currentScript
  ? document.currentScript.src.replace(/[^\/]*?\.js$/, "")
  : "";

Hyweb.ComponentsGroup = {}; //! 组件组
Hyweb.BlocksGroup = {}; //! 块组
...
*/
  ...
  render: function(type) {
    ...
    if (!(Hyweb.preservePropertySections && section.length)) {
    ...
    if (component.beforeInit)
      component.beforeInit(Hyweb.Builder.selectedEl.get(0));

    var element;

    var fn = function(component, property) {
      return property.input.on("propertyChange", function(event, value, input) {
        var element = Hyweb.Builder.selectedEl;
        ...
        if (property.htmlAttr) {
          ...
          if (property.htmlAttr == "class" && property.validValues) {
            ...
            element = Hyweb.StyleManager.setStyle(element, property.key, value);
          } else if (property.htmlAttr == "innerHTML") {
            element = Hyweb.ContentManager.setHtml(element, value);
          } else {
            element = element.attr(property.htmlAttr, value);
          }

          Hyweb.Undo.addMutation({
            type: "attributes",
        ...
        if (!property.child && !property.parent)
          Hyweb.Builder.selectNode(element);

        return element;
      });
    };

    var nodeElement = Hyweb.Builder.selectedEl;
    for (var i in component.properties) {
      ...
      if (property.init) {
        ...
        if (property.htmlAttr == "style") {
          ...
          var value = Hyweb.StyleManager.getStyle(element, property.key); //getStyle returns declared style
        } else if (property.htmlAttr == "innerHTML") {
          var value = Hyweb.ContentManager.getHtml(element);
        } else {
      ...
      // ! i don't konw  what this is , I don't understand it at all, Fool leadership aasked me to research
    ...
    if (component.init) component.init(Hyweb.Builder.selectedEl.get(0));
  }
};
...
Hyweb.Builder = {
  ...
  loadControlGroups: function() {
    ...
    componentsList.each(function() {
      ...
      for (group in Hyweb.ComponentsGroup) {
        list.append(
        ...
        components = Hyweb.ComponentsGroup[group];

        for (i in components) {
          componentType = components[i];
          component = Hyweb.Components.get(componentType);
          ...
          if (component) {
            ...
            if (component.image) {
              item.css({
                backgroundImage:
                  "url(" + "libs/builder/" + component.image + ")",
  ...
  loadBlockGroups: function() {
    ...
    blocksList.each(function() {
      ...
      for (group in Hyweb.BlocksGroup) {
        list.append(
        ...
        blocks = Hyweb.BlocksGroup[group];

        for (i in blocks) {
          blockType = blocks[i];
          block = Hyweb.Blocks.get(blockType);
          ...
          if (block) {
            ...
            if (block.image) {
              item.css({
                backgroundImage:
                  "url(" +
                  (block.image.indexOf("//") == -1 ? "libs/builder/" : "") +
                  block.image +
                  ")",
                backgroundRepeat: "no-repeat"
              });
  ...
  loadUrl: function(url, callback) {
    ...
    self.initCallback = callback;
    if (Hyweb.Builder.iframe.src != url) Hyweb.Builder.iframe.src = url;
  },
  ...
  _loadIframe: function(url) {
    ...
    return this.documentFrame.on("load", function() {
      ...
      Hyweb.WysiwygEditor.init(window.FrameDocument);
  ...
  loadNodeComponent: function(node) {
    data = Hyweb.Components.matchNode(node);
    var component;

    if (data) component = data.type;
    else component = Hyweb.defaultComponent;

    Hyweb.Components.render(component);
  },
  ...
  selectNode: function(node) {
    ...
    if (self.texteditEl && self.selectedEl.get(0) != node) {
      Hyweb.WysiwygEditor.destroy(self.texteditEl);
      jQuery("#select-box")
  ...
  _initHighlight: function() {
          ...
          try {
            if (event.originalEvent) {
              if (offset.top < y - halfHeight || offset.left < x - halfWidth) {
                if (isIE11) self.highlightEl.append(self.dragElement);
                else self.dragElement.appendTo(parent);
              } else {
                if (isIE11) self.highlightEl.prepend(self.dragElement);
        ...
        } // else //! 取消注释else以在拖动时禁用父级突出显示
        {
          jQuery("#highlight-box").css({
            ...
            display: event.target.hasAttribute("contenteditable")
              ? "none"
              : "block",
            border: self.isDragging ? "1px dashed aqua" : "" //!拖动带有绿色的高亮父控件时
          });
          ...
          else jQuery("#highlight-name").show(); //hide tag name when dragging  //! 拖动时隐藏标记名称
        }
      }
    });
      ...
      if (self.isDragging) {
        ...
        if (self.dragMoveMutation === false) {
          if (self.component.dragHtml) {
            //if dragHtml is set for dragging then set real component html //! 如果dragHtml设置为拖动，那么设置真正的组件html
        ...
        if (self.dragMoveMutation === false) {
          ...
          Hyweb.Undo.addMutation(self.dragMoveMutation);
    ...
    self.frameHtml.on("dblclick", function(event) {
      if (Hyweb.Builder.isPreview == false) {
        self.texteditEl = target = jQuery(event.target);

        Hyweb.WysiwygEditor.edit(self.texteditEl);
  ...
  _initBox: function() {
    ...
    $(".components-list li ol li", addSectionBox).on("click", function(event) {
      var html = Hyweb.Components.get(this.dataset.type).html;

      addSectionComponent(
        html,
        jQuery("[name='add-section-insert-mode']:checked").val() == "after"
      );
    ...
    $(".blocks-list li ol li", addSectionBox).on("click", function(event) {
      var html = Hyweb.Blocks.get(this.dataset.type).html;
    ...
    $(".drag-elements-sidepane ul > li > ol > li").on(
      ...
      function(event) {
        ...
        if ($this.data("drag-type") == "component")
          self.component = Hyweb.Components.get($this.data("type"));
        else self.component = Hyweb.Blocks.get($this.data("type"));
  ...
  saveAjax: function(fileName, startTemplateUrl, callback) {
    var data = {};
    data["fileName"] =
      fileName && fileName != "" ? fileName : Hyweb.FileManager.getCurrentUrl();
...
Hyweb.CodeEditor = {
  ...
  init: function(doc) {
    $("#vvveb-code-editor textarea").val(Hyweb.Builder.getHtml());

    $("#vvveb-code-editor textarea").keyup(function() {
      delay(Hyweb.Builder.setHtml(this.value), 1000);
    });

    //load code on document changes
    Hyweb.Builder.frameBody.on("vvveb.undo.add vvveb.undo.restore", function(e) {
      Hyweb.CodeEditor.setValue();
    });
    //load code when a new url is loaded
    Hyweb.Builder.documentFrame.on("load", function(e) {
      Hyweb.CodeEditor.setValue();
    });
...
Hyweb.Gui = {
  init: function() {
    $("[data-vvveb-action]").each(function() {
      ...
      $(this).on(on, Hyweb.Gui[this.dataset.vvvebAction]);
      if (this.dataset.vvvebShortcut) {
        $(document).bind(
          "keydown",
          this.dataset.vvvebShortcut,
          Hyweb.Gui[this.dataset.vvvebAction]
        );
  ...
  undo: function() {
    if (Hyweb.WysiwygEditor.isActive) {
      Hyweb.WysiwygEditor.undo();
    } else {
      Hyweb.Undo.undo();
    }
    Hyweb.Builder.selectNode();
  },

  redo: function() {
    if (Hyweb.WysiwygEditor.isActive) {
      Hyweb.WysiwygEditor.redo();
    } else {
      Hyweb.Undo.redo();
    }
    Hyweb.Builder.selectNode();
  ...
  save: function() {
    $("#textarea-modal textarea").val(Hyweb.Builder.getHtml());
    $("#textarea-modal").modal();
  },

  //post html content through ajax to save to filesystem/db
  //! 通过ajax发布html内容以保存到文件系统/数据库
  saveAjax: function() {
    var url = Hyweb.FileManager.getCurrentUrl();

    return Hyweb.Builder.saveAjax(url, null, function(data) {
      $("#message-modal")
        .modal()
        .find(".modal-body")
  ...
  download: function() {
    ...
    uriContent =
      ...
      encodeURIComponent(Hyweb.Builder.getHtml());
  ...
  toggleEditor: function() {
    ...
    Hyweb.CodeEditor.toggle();
  },

  toggleEditorJsExecute: function() {
    Hyweb.Builder.runJsOnSetHtml = this.checked;
  },

  preview: function() {
    Hyweb.Builder.isPreview == true
      ? (Hyweb.Builder.isPreview = false)
      : (Hyweb.Builder.isPreview = true);
    $("#iframe-layer").toggle();
  ...
  newPage: function() {
    ...
    newPageModal
      ...
      .submit(function(event) {
        ...
        //! 只允许字母数字，点字符扩展(如。html)和/允许键入完整的路径，包括文件夹
        fileName = fileName.replace(/[^A-Za-z0-9\.\/]+/g, "-").toLowerCase();
        //add your server url/prefix/path if needed
        //! 如果需要，添加您的服务器url/前缀/路径
        var url = "" + fileName;
        Hyweb.FileManager.addPage(name, title);
        event.preventDefault();

        return Hyweb.Builder.saveAjax(url, startTemplateUrl, function(data) {
          Hyweb.FileManager.loadPage(name);
          Hyweb.FileManager.scrollBottom();
  ...
  setDesignerMode: function() {
    ...
    //! 调用action后，aria-pressed属性将被更新，我们将检查是否为false而不是true
    var designerMode = this.attributes["aria-pressed"].value != "true";
    Hyweb.Builder.setDesignerMode(designerMode);
  },
  //layout
  ...
  toggleFileManager: function() {
    Hyweb.Gui.togglePanel("#filemanager", "--builder-filemanager-height");
  },

  toggleLeftColumn: function() {
    Hyweb.Gui.togglePanel("#left-panel", "--builder-left-panel-width");
  },

  toggleRightColumn: function() {
    Hyweb.Gui.togglePanel("#right-panel", "--builder-right-panel-width");
    ...
    Hyweb.Components.componentPropertiesElement =
...
Hyweb.FileManager = {
  ...
  init: function() {
    ...
    $(this.tree).on("click", "li[data-page] label", function(e) {
      ...
      if (page) Hyweb.FileManager.loadPage(page);
      return false;
    });
    ...
    $(this.tree)
      .on("click", "li[data-component] label ", function(e) {
        ...
        Hyweb.Builder.frameHtml.animate(
          {
        ...
        Hyweb.Builder.selectNode(node);
        Hyweb.Builder.loadNodeComponent(node);
      ...
      if (node.hasChildNodes()) {
        for (var j = 0; j < node.childNodes.length; j++) {
          ...
          if (
            ...
            (matchChild = Hyweb.Components.matchNode(child))
          ) {
            if (
  ...
  loadPage: function(name, allowedComponents = false, disableCache = true) {
    ...
    Hyweb.Builder.loadUrl(
      url +
        (disableCache
          ? (url.indexOf("?") > -1 ? "&" : "?") + Math.random()
          : ""),
      function() {
        Hyweb.FileManager.loadComponents(allowedComponents);
      }
    );
  },
